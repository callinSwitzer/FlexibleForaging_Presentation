...
for(ii in 1:length(bees)){
  ...
  db_results = dbscan(as.matrix(data_$freq), eps = 27)
...
# make a new df4, and redo stats to see if there is a difference
newdf3 <- newdf5
...
m7 <- update(m6, .~. - visitNum_positive)
BIC(m7, m6) %>%
  mutate(model = row.names(.),
    keep = recode(as.character(order(.$BIC) == 1),
                   "TRUE" = "*",
...
# refref: frequency model that has the same predictors as accel model
...
ma6 <- update(ma5, .~. - visitNum_positive)
BIC(ma5, ma6) %>%
  mutate(model = row.names(.),
    keep = recode(as.character(order(.$BIC) == 1),
                   "TRUE" = "*",
